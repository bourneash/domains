...
| [`fleet-backup/`](./fleet-backup/) | Offsite R2 snapshots of the fleet's unreproducible state (cf-stats history, data-hub + site-tracker DBs, Gatus uptime, the dashboard action log). Docker volumes included; secrets abort the run; `--drill` proves archives restore. |
