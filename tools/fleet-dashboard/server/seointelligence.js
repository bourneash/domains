  ...
  for (const row of aggregateQueries(records)) {
    ...
    if (pos >= 5 && pos <= 20) {
      const score = Math.min(
        100,
        Math.round(28 + Math.log10(row.impressions + 1) * 16 + (20 - pos) * 1.8)
      );
      actions.push({
        ...
        recommendation:
          'Identify the ranking page, strengthen its intent match and internal links, then monitor the query for 28 days.',
    ...
    if (pos <= 10 && row.impressions >= 50 && row.ctr < expectedCtr) {
      const score = Math.min(
        100,
        Math.round(35 + Math.log10(row.impressions + 1) * 15 + (expectedCtr - row.ctr) * 600)
      );
      actions.push({
        ...
        recommendation:
          'Review the ranking page title and description against the live search intent; make the benefit more specific without changing the URL.',
        metric: { label: 'impressions', value: row.impressions },
        query: row.query,
      });
    }
  ...
  return [
    {
      ...
      recommendation:
        'Compare losing queries and pages, then check recent deploys, indexing, content changes, and SERP displacement before editing.',
...
function pageActions(site, gscRecords, ga4Records) {
  ...
  for (const row of search.values()) {
    if (row.impressions < 50 || row.position == null || row.position < 4 || row.position > 20)
      continue;
    ...
    actions.push({
      id: `${site}:page-opportunity:${row.page}`,
      site,
      ...
      recommendation:
        'Strengthen this page around the search demand it already earns, improve its internal-link support, and preserve any converting intent.',
  ...
  for (const row of analytics.values()) {
    if (
      ...
      row.engagementRate >= 0.35
    )
      continue;
    const valueScore = pageValueScore(row);
    const score = Math.min(
      100,
      Math.round(45 + Math.log10(row.sessions + 1) * 12 + (0.35 - row.engagementRate) * 45)
    );
    actions.push({
      id: `${site}:engagement-risk:${row.page}`,
      site,
      ...
      recommendation:
        'Check intent alignment, first-screen clarity, navigation paths, and conversion placement before sending more traffic to this page.',
...
function queryPageActions(site, records, ga4Records) {
  ...
  for (const row of pairs) {
    ...
    const score = Math.min(
      100,
      Math.round(48 + Math.log10(row.impressions + 1) * 11 + estimatedClicks * 1.5)
    );
    actions.push({
      id: `${site}:click-uplift:${row.query}:${row.page}`,
      site,
      type: 'click-uplift',
      priority: priorityForScore(score),
      score,
      ...
      recommendation:
        'Use the exact ranking URL and query to improve SERP appeal and intent coverage without changing the canonical URL.',
  ...
  for (const [query, rows] of byQuery) {
    const viable = rows
      .filter(row => row.impressions >= 5 && row.position <= 50)
      .sort(
    ...
    if (
      viable.length < 2 ||
      totalImpressions < 30 ||
      viable[1].impressions < totalImpressions * 0.15
    )
      continue;
    ...
    const valueScore = pageValueScore({
      ...
      conversions: viable.reduce(
        (sum, row) => sum + (analytics.get(row.page)?.conversions || 0),
        0
      ),
    });
    const score = Math.min(
...
function pageDecayActions(site, ga4Records, now = new Date()) {
  ...
  for (const row of byPage.values()) {
    if (
      row.currentDays.size < 7 ||
      row.previousDays.size < 7 ||
      row.previous < 30 ||
      row.current >= row.previous * 0.7
    )
      continue;
    ...
    actions.push({
      id: `${site}:content-decay:${row.page}`,
      site,
      type: 'content-decay',
      priority: priorityForScore(score),
      score,
      ...
      recommendation:
        'Determine whether rankings, demand, freshness, or a site change caused the loss; refresh only after isolating the cause.',
...
function actionPlan(action) {
  const plans = {
    'page-opportunity': [
      'Inspect the page’s leading queries and competing results.',
      'Improve intent coverage and add relevant internal links.',
      'Recheck impressions, position, clicks, and conversions after 28 days.',
    ],
    'content-decay': [
      'Compare losing dates, queries, and deploy history.',
      'Separate demand loss from ranking, indexing, and content causes.',
      'Apply the smallest corrective change and verify against the baseline.',
    ],
    'engagement-risk': [
      'Review source intent and the first screen on mobile.',
      'Clarify the next useful action and remove interaction friction.',
      'Measure engagement and conversions after a full traffic cycle.',
    ],
    'click-uplift': [
      'Compare the exact query-page snippet with adjacent results.',
      'Improve the title, description, and above-fold intent match without changing the URL.',
      'Measure CTR and clicks against the recorded query-page baseline after 28 days.',
    ],
    cannibalization: [
      'Confirm whether the competing pages satisfy the same intent.',
      'Choose a primary URL, then consolidate overlap or differentiate the secondary pages.',
      'Verify that impressions concentrate on the intended page without losing total clicks.',
    ],
    'striking-distance': [
      'Identify the ranking page and competing result format.',
      'Close the intent gap and strengthen contextual internal links.',
      'Track the query for 28 days without changing its URL.',
    ],
    'low-ctr': [
      'Compare the current snippet with neighboring results.',
      'Rewrite title and description around a specific benefit.',
      'Monitor CTR while holding the URL and page intent stable.',
    ],
    'traffic-decline': [
      'Segment losses by query and landing page.',
      'Check indexing, deploys, SERP changes, and seasonality.',
      'File targeted fixes only for the confirmed cause.',
    ],
    'web-vitals': [
      'Profile the slowest shared template on mobile.',
      'Fix the dominant image, font, script, or rendering bottleneck.',
      'Rerun the pinned fleet measurement and compare each breached metric.',
    ],
    'broken-links': [
      'Export each failing source and destination pair.',
      'Replace internal links with the final canonical destination.',
      'Rerun link-rot and confirm zero remaining failures.',
    ],
    crawlability: [
      'Restore a valid sitemap of canonical indexable URLs.',
      'Reference it from robots.txt and verify successful retrieval.',
      'Rerun the crawl and confirm discovered page coverage.',
    ],
  };
  return (
    plans[action.type] || [
      action.recommendation,
      'Verify the result against the recorded baseline.',
    ]
  );
}
...
function webVitalsActions(report) {
  ...
  for (const row of report?.sites || []) {
    ...
    const evidence = [];
    if (row.budget_breaches.includes('performance'))
      evidence.push(`performance ${Math.round((m.performance || 0) * 100)}`);
    ...
    actions.push({
      ...
      recommendation:
        'Profile the slowest template and fix the shared rendering, image, font, or script bottleneck; verify against the pinned fleet baseline.',
  ...
  for (const row of report?.sites || []) {
    if (row.error) {
      actions.push({
        ...
        evidence: row.error,
        recommendation:
          'Publish a valid sitemap containing canonical, indexable URLs and reference it from robots.txt, then rerun link-rot.',
    ...
    const broken = findings.filter(f =>
      ['broken', 'unreachable', 'page-unreachable'].includes(f.kind || f.type)
    );
    ...
    actions.push({
      ...
      recommendation:
        'Update internal links to the final live URL; remove dead destinations and rerun the fleet link sweep.',
...
function buildSiteRows(
  ...
  for (const site of siteNames) {
    ...
    row.pages = new Set([
      ...gscPages.map(page => page.page),
      ...ga4Pages.map(page => page.page),
    ]).size;
...
async function buildSnapshot({
  ...
  const siteValueScores = Object.fromEntries(
    siteNames.map(site => {
      const totals = aggregatePages(site, ga4PageData[site], 'ga4').reduce(
        (out, row) => ({
  ...
  const searchActions = siteNames.flatMap(site => {
    ...
    const broadQueries = queryActions(site, queryData[site]).filter(
      action => !exactQueries.has(action.query)
    );
    const broadPages = pageActions(site, gscPageData[site], ga4PageData[site]).filter(
      action => action.type !== 'page-opportunity' || !exactPages.has(action.page)
    );
  ...
  const hasAnalyticsEvidence =
    Object.values(queryData).some(rows => rows.length) ||
    Object.values(ga4PageData).some(rows => rows.length) ||
    Object.values(queryPageData).some(rows => rows.length);
  const value = {
    generatedAt: now.toISOString(),
    windowDays: days,
    upstream: {
      ok: !upstreamError,
      partial: Boolean(upstreamError && hasAnalyticsEvidence),
      error: upstreamError,
    },
    sources: {
      analyticsConfigured: siteNames.length,
      gscReady: Object.values(health.sites || {}).filter(s =>
        (s.gsc?.status || '').startsWith('ok')
      ).length,
      ga4Ready: Object.values(health.sites || {}).filter(s =>
        (s.ga4?.status || '').startsWith('ok')
      ).length,
    ...
    totals: {
      ...
      conversions: Object.entries(ga4PageData).reduce(
        (total, [site, rows]) =>
          total + aggregatePages(site, rows, 'ga4').reduce((sum, row) => sum + row.conversions, 0),
        0
      ),
      queryPagePairs: Object.entries(queryPageData).reduce(
        (total, [site, rows]) => total + aggregateQueryPages(site, rows).length,
        0
      ),
