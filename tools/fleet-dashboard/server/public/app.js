...
async function renderExecutive() {
  ...
  let messages, proposals, actions, settings, brief, revops, experiments;
  try {
    [messages, proposals, actions, settings, brief, revops, experiments] = await Promise.all([
  ...
  app.innerHTML = `${executiveBreadcrumb}<div class="page-head"><div><h2 class="page-title">CEO / CTO Executive Office</h2><div class="muted">Autonomous business leadership, owner approvals, operating context, and audit history</div></div><button class="btn" id="ex-refresh">↻ Refresh</button></div>
    ...
    <section class="card" style="margin-bottom:12px"><h3>Revenue operating system</h3><div class="muted">The CEO can now track the funnel and run measured experiments instead of treating traffic as revenue.</div><div class="stat-grid" style="margin-top:10px"><div><b>${esc(revopsSummary.total_leads ?? 0)}</b><span>tracked leads</span></div><div><b>${esc(revopsSummary.mqls ?? 0)}</b><span>MQLs</span></div><div><b>${esc(revopsSummary.opportunities ?? 0)}</b><span>opportunities</span></div><div><b>${esc(experimentRows.filter(row => row.state === 'running').length)}</b><span>running experiments</span></div></div><p class="muted" style="margin-bottom:0">Lead and experiment APIs are available to Fleet Manager integrations and are included in future executive briefs.</p></section>
