...
function tasksDir(root, slug) {
  return path.join(siteDir(root, slug), 'ops', 'tasks');
}
...
function get(root, slug, column, file) {
  if (!isValidColumn(column) || !isValidFilename(file))
    throw httpErr(400, 'bad column or filename');
...
function create(root, slug, column, payload) {
  ...
  const meta = {
    ...
    priority:
      payload.priority != null && payload.priority !== '' ? Number(payload.priority) : undefined,
    type: payload.type || undefined,
    estimated_turns:
      payload.estimated_turns != null && payload.estimated_turns !== ''
        ? Number(payload.estimated_turns)
        : undefined,
...
function update(root, slug, column, file, payload) {
  ...
  meta.assigned_role = assignedRoleForType(meta.type, meta.assigned_role);
...
function move(root, slug, fromCol, file, toCol) {
  if (!isValidColumn(fromCol) || !isValidColumn(toCol) || !isValidFilename(file))
    throw httpErr(400, 'bad column or filename');
  ...
  if (toCol === 'in-progress' && !parsed.meta.started_at)
    parsed.meta.started_at = new Date().toISOString();
  if (toCol === 'done' && !parsed.meta.completed_at)
    parsed.meta.completed_at = new Date().toISOString();
...
function remove(root, slug, column, file) {
  ...
  let n = 2;
  while (fs.existsSync(dest)) {
    name = `${column}__${file.replace(/\.md$/, `-${n}.md`)}`;
