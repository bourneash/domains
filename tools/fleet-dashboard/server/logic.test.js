...
test('roleFromCommand recognizes worker and dedicated-script roles', () => {
  assert.deepEqual(parse.roleFromCommand('cd /work && bash ops/scripts/run-worker.sh engineer'), {
    role: 'engineer',
    worker: true,
  });
  assert.deepEqual(parse.roleFromCommand('bash ops/scripts/run-deployer.sh'), {
    role: 'deployer',
    worker: false,
  });
...
test('roles.parseRoles and cron parseCrontab agree on role recognition', () => {
  ...
  const viaCron = parse
    ...
    .entries.filter(e => !e.commented && e.role)
    .map(e => e.role)
    .sort();
  assert.deepEqual(viaCron, ['deployer', 'engineer']);
});

test('roles.parseRoles can expose commented roles as enrolled-but-paused', () => {
  const crontab =
    '# 1,6,11,16,21,26,31,36,41,46,51,56 * * * * bash ops/scripts/run-principal-engineer.sh\n';
  assert.deepEqual(roles.parseRoles(crontab), []);
  assert.deepEqual(roles.parseRoles(crontab, { includeCommented: true }), [
    {
      ...
      commented: true,
      lineIndex: 0,
      rawLine: crontab.trimEnd(),
    },
  ]);
});

test('the 3boobs Principal Engineer schedule is represented as paused enrollment', async () => {
  const root = path.resolve(__dirname, '..', '..', '..');
  const matrix = await roles.matrix(root, ['3boobs.com']);
  const cell = matrix.sites.find(s => s.site === '3boobs.com')?.cells?.['principal-engineer'];
  assert.deepEqual(
    { enabled: cell?.enabled, state: cell?.state, commented: cell?.commented },
    { enabled: false, state: 'paused', commented: true }
  );
...
test('computeSyncState classifies upstream sync correctly', () => {
  ...
  assert.equal(
    git.computeSyncState({ ahead: 0, behind: 2, upstream: 'origin/main' }),
    'diverged-behind'
  );
  assert.equal(
    git.computeSyncState({ ahead: 1, behind: 2, upstream: 'origin/main' }),
    'diverged-behind'
  );
...
test('remoteToWebUrl converts scp-syntax, ssh://, and http(s) remotes; rejects unknown shapes', () => {
  assert.equal(
    git.remoteToWebUrl('git@github-bourneash:bourneash/xxxtea.com.git'),
    'https://github.com/bourneash/xxxtea.com'
  );
  assert.equal(
    git.remoteToWebUrl('git@github.com:bourneash/americastrikes.git'),
    'https://github.com/bourneash/americastrikes'
  );
  assert.equal(
    git.remoteToWebUrl('ssh://git@github-bourneash/bourneash/saveusfarms.com.git'),
    'https://github.com/bourneash/saveusfarms.com'
  );
  assert.equal(
    git.remoteToWebUrl('https://github.com/bourneash/reviewtattoo.git'),
    'https://github.com/bourneash/reviewtattoo'
  );
  assert.equal(
    git.remoteToWebUrl('git@gitlab.example.com:team/repo.git'),
...
test('remoteToWebUrl strips embedded credentials from an http(s) remote', () => {
  assert.equal(
    git.remoteToWebUrl(
      'https://x-access-token:gho_secrettoken123@github.com/bourneash/sinderella.git'
    ),
...
test('siteCronContainer prefers compose container_name, falls back to stem', () => {
  ...
  const mk = (slug, compose) => {
    ...
    if (compose != null)
      fs.writeFileSync(path.join(root, 'sites', slug, 'docker-compose.yml'), compose);
  };
...
test('parseLocalBranches parses for-each-ref tab output', () => {
  const out = ['main\torigin/main\t*', 'feature/foo\torigin/feature/foo\t ', 'scratch\t\t '].join(
    '\n'
  );
...
test('parseRemoteOnlyBranches excludes the origin symref and branches that exist locally', () => {
  ...
  assert.deepEqual(git.parseRemoteOnlyBranches(remoteOut, localNames), [
    { name: 'origin/stray-remote-branch' },
  ]);
});
