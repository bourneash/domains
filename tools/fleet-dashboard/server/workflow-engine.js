...
function buildGraph(items = [], links = []) {
  ...
  for (const link of links) {
    ...
    if (link.relation === 'blocks') {
      prerequisite = key(link.from_type, link.from_id);
      dependent = key(link.to_type, link.to_id);
    } else if (link.relation === 'blocked_by') {
      prerequisite = key(link.to_type, link.to_id);
      dependent = key(link.from_type, link.from_id);
    } else continue;
...
function evaluate({ items = [], links = [], now = new Date() } = {}) {
  ...
  const unresolved = node =>
    [...(graph.prerequisites.get(node) || [])].filter(
      prereq => !TERMINAL.has(graph.nodes.get(prereq)?.status)
    );
  const effort = item =>
    Math.max(1, Number(item?.estimate_minutes || item?.effort_minutes || 60) || 60);
  const longest = (node, seen = new Set()) => {
    if (seen.has(node)) return 0;
    const nextSeen = new Set(seen).add(node);
    return (
      effort(graph.nodes.get(node)) +
      Math.max(0, ...[...(graph.dependents.get(node) || [])].map(child => longest(child, nextSeen)))
    );
  };
  for (const [node, item] of graph.nodes) {
    ...
    nodes[node] = {
      ...
      status: item.status,
      blockers,
      ready: !blockers.length && !cycles.some(cycle => cycle.includes(node)),
      effort_minutes: effort(item),
      critical_score: priority * 60 + longest(node),
      critical: false,
    };
  }
  const maxScore = Math.max(0, ...Object.values(nodes).map(node => node.critical_score));
  Object.values(nodes).forEach(node => {
    node.critical = node.critical_score === maxScore && maxScore > 0;
  });
  const alerts = [];
  cycles.forEach(cycle =>
    alerts.push({
      ...
      nodes: cycle,
      dedupe_key: `workflow:cycle:${cycle.slice().sort().join('|')}`,
      message: `Cycle detected: ${cycle.join(' → ')}`,
    })
  );
  Object.values(nodes).forEach(node => {
    if (node.blockers.length && !TERMINAL.has(node.status))
      alerts.push({
        kind: 'blocked',
        severity: 'high',
        node: node.key,
        title: node.title,
        dedupe_key: `workflow:blocked:${node.key}:${node.blockers.slice().sort().join('|')}`,
        message: `${node.title} is blocked by ${node.blockers.join(', ')}`,
      });
    const item = graph.nodes.get(node.key);
    if (item?.due_at && new Date(item.due_at) < now && !TERMINAL.has(item.status))
      alerts.push({
        kind: 'overdue',
        severity: 'high',
        node: node.key,
        title: item.title,
        dedupe_key: `workflow:overdue:${node.key}:${item.due_at}`,
        message: `Due ${item.due_at}`,
      });
  });
  const edges = [];
  for (const [dependent, prerequisites] of graph.prerequisites)
    for (const prerequisite of prerequisites)
      edges.push({ from: prerequisite, to: dependent, relation: 'blocks' });
  return {
    nodes,
    edges,
    cycles,
    alerts,
    critical_path: Object.values(nodes)
      .filter(node => node.critical)
      .sort((a, b) => b.critical_score - a.critical_score)
      .map(node => node.key),
  };
}
