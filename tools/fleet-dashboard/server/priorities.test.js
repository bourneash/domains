...
function fixture() {
  ...
  fs.mkdirSync(path.join(root, 'sites', 'live.example', 'ops', 'tasks', 'backlog'), { recursive: true });
  fs.writeFileSync(path.join(root, 'sites', 'live.example', 'ops', 'tasks', 'backlog', 'work.md'), `---\ntitle: Do work\nassigned_role: missing-role\n---\nBody\n`);
  return root;
}
...
test('joins lifecycle, analytics gaps, task ownership and growth actions', () => {
  ...
  const out = priorities.build({
    root, discoveredSites: ['live.example'], analyticsHealth: { sites: {} }, revenue: { connected: false },
    seo: { actions: [{ key: 'abc', site: 'live.example', title: 'Grow page', evidence: '100 impressions', rankScore: 77, valueScore: 42, filed: false }] },
  });
...
test('does not demand production coverage from scaffold sites', () => {
  const root = fixture();
  const out = priorities.build({ root, discoveredSites: ['live.example'], analyticsHealth: { sites: { 'live.example': {} } }, revenue: {}, seo: { actions: [] } });
