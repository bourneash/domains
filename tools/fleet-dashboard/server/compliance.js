...
function saveState() {
  ...
  try {
    ...
    /* best-effort cache; scanning must still succeed */
  }
}
...
function elementText(source, tag, words) {
  ...
  return (
    quotedStrings(source, 80).find(value => {
...
function bannerWording(source) {
  const direct =
    /['"]([^'"]{0,20}(?:we (?:use|store)|this (?:site )?uses)[^'"]{0,500}(?:cookie|consent)[^'"]{0,300})['"]/i.exec(
      source
    );
  if (direct) return short(direct[1]);
  const strings = quotedStrings(source).filter(
    value =>
      value.length >= 25 &&
  ...
  strings.sort((a, b) => {
    const score = s =>
      (/\bwe (?:use|store)\b/i.test(s) ? 100 : 0) +
      (/privacy-respecting|see which pages|get read|no ad tracking/i.test(s) ? 300 : 0) +
      (/analytics cookie/i.test(s) ? 100 : 0) +
      Math.min(s.length, 180);
    return score(b) - score(a);
  });
  if (strings[0]) return short(strings[0]);
  const blocks =
    /<(?:div|section|aside)\b[^>]*(?:cookie|consent|banner|dialog|modal)[^>]*>([\s\S]{0,3000}?)<\/(?:div|section|aside)>/gi;
...
function legalLink(source, kind, pageUrl) {
  ...
  if (routeRe.test(source) && contentRe.test(source)) {
    try {
      return new URL(route, pageUrl).href;
    } catch {
      return route;
    }
  }
...
function analyze(text, { url = '' } = {}) {
  ...
  const measurementIds = [...new Set(source.match(/\bG-[A-Z0-9]{10}\b/gi) || [])].map(id =>
    id.toUpperCase()
  );
  const ga4 =
  ...
  const reject = has(
    /reject(?:\s+all)?|decline(?:\s+all)?|deny|necessary only|essential only/i,
    lower
  );
  const banner =
    cookieLanguage &&
    accept &&
    has(
      /banner|dialog|modal|cookie-consent|cookie_banner|consent-banner|consentmanager|cookiebot/i,
      lower
    );
  const defaultDenied =
  ...
  const consentUpdate = has(
    /gtag\s*\(\s*['"]consent['"]\s*,\s*['"]update['"]|updateConsentState/i,
    source
  );
  const basicGate =
    ga4 &&
    has(/consent|cookie/i, lower) &&
    has(
      /localStorage|sessionStorage|document\.cookie|consent-granted|analytics_consent|cookie_consent/i,
      source
    ) &&
    has(
      /load(?:Analytics|GA|Gtag)|appendChild\s*\(|createElement\s*\(\s*['"]script|googletagmanager/i,
      source
    );
  ...
  const evidence = {
    ...
    rejectLabel: elementText(
      source,
      '(?:button|a)',
      /reject(?:\s+all)?|decline(?:\s+all)?|necessary only|essential only/i
    ),
    privacyUrl,
    termsUrl,
  };
  ...
  if (ga4 && !gaConsentGated)
    failures.push('GA4 detected without default-denied or basic consent gating');
...
async function fetchText(url, { requireHtml = false } = {}) {
  ...
  if (!response.ok)
    throw new ScanError(`HTTP ${response.status}`, 'http', { httpStatus: response.status });
...
async function scanSite(site) {
  ...
  try {
    ...
    const errorType =
      error.type ||
      (/timeout|abort/i.test(`${error.name || ''} ${message}`)
        ? 'timeout'
        : /ENOTFOUND|EAI_AGAIN|DNS/i.test(`${causeCode || ''} ${message}`)
          ? 'dns'
          : /certificate|CERT_|TLS|SSL/i.test(`${causeCode || ''} ${message}`)
            ? 'tls'
            : 'network');
    return {
      site,
...
// A site may be onboarded while the dashboard process is already running.
// Scan newly discovered sites promptly instead of waiting for the hourly fleet
// refresh to replace the "not scanned yet" placeholder.
function scanMissing(sites) {
  const missing = [...new Set(sites)].filter(site => !CACHE[site]);
  return missing.length ? scanAll(missing) : Promise.resolve([]);
}
...
function progress() {
  return { ...PROGRESS, currentSites: [...PROGRESS.currentSites] };
}

function fleetHistory(sites, limit = 20) {
  const events = sites
    .flatMap(site => (HISTORY[site] || []).map(entry => ({ site, ...entry })))
  ...
  return [...buckets.entries()]
    ...
    .map(([at, statuses]) => {
      ...
      return {
        ...
        pass,
        total: values.length,
        passRate: values.length ? Math.round((100 * pass) / values.length) : 0,
      };
    });
}
