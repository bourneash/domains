...
const TRANSITIONS = {
  ...
  proven: [], inconclusive: [], cancelled: [], 'rolled-back': [],
};
...
function start({ store, root, site, action, baseline = {} }) {
  if (!action || !action.key || action.site !== site) throw httpErr(400, 'invalid improvement action');
  const duplicate = store.listImprovements({ source: 'seo-intelligence', source_id: action.key, limit: 10 })
  ...
  const file = tasks.create(root, site, 'backlog', {
    task_id: taskId, title: action.title,
    priority: action.priority === 'high' ? 1 : action.priority === 'medium' ? 2 : 3,
    type: 'site-improvement', estimated_turns: action.priority === 'high' ? 3 : 2,
    assigned_role: ['web-vitals', 'broken-links', 'crawlability'].includes(action.type) ? 'engineer' : 'seo-analyst',
    source: 'improvement-workbench', source_id: action.key, correlation_id: correlationId,
    measurement_due: due,
  ...
  store.record({ event_type: 'improvement.started', source: 'improvement-workbench',
    site_id: `site:${site}`, entity_type: 'improvement', entity_id: runId,
    correlation_id: correlationId, payload: { task_id: taskId, task_file: file, source_id: action.key } });
...
function transition(store, runId, input = {}) {
  ...
  if (state === 'review' && current.validation?.passed !== true && input.validation?.passed !== true)
  ...
  store.record({ event_type: `improvement.${state}`, source: 'improvement-workbench',
    site_id: `site:${run.site}`, entity_type: 'improvement', entity_id: runId,
    correlation_id: run.correlation_id, payload: { from: current.state, ...patch } });
  return run;
}
...
function compareOutcome(baseline, current, measuredAt = new Date().toISOString()) {
  ...
  for (const key of keys) {
    if (!Number.isFinite(Number(baseline?.[key])) || !Number.isFinite(Number(current?.[key]))) continue;
    const before = Number(baseline[key]), after = Number(current[key]);
    deltas[key] = { before, after, absolute: after - before,
