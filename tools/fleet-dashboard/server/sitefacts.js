...
const COMPLIANCE_FILE =
  process.env.FD_COMPLIANCE_FILE || path.join(DATA_DIR, 'compliance-latest.json');
const AMZ_LATEST =
  process.env.FD_AMZ_LATEST || path.join(__dirname, '..', '..', 'amz-stats', 'out', 'latest.json');
...
const FACTS = [
  {
    key: 'seo.has_canonical',
    family: 'seo',
    describe: '<link rel="canonical"> present',
    kind: 'head_regex',
  ...
  },
  {
    key: 'seo.has_og_tags',
    family: 'seo',
    describe: 'Open Graph meta tags present',
    kind: 'head_regex',
    pattern: /property=["']og:(title|image|description)["']/i,
    rule: 'green_yellow',
  },
  {
    key: 'seo.has_schema_org',
    family: 'seo',
    describe: 'schema.org structured data present',
    kind: 'head_regex',
    pattern: /application\/ld\+json|itemscope/i,
    rule: 'green_yellow',
  },
  {
    key: 'seo.has_viewport_meta',
    family: 'seo',
    describe: '<meta name="viewport"> present',
    kind: 'head_regex',
    pattern: /name=["']viewport["']/i,
    rule: 'green_yellow',
  },
  {
    key: 'seo.has_rss',
    ...
    kind: 'head_regex',
    pattern: /type=["'](application\/(rss|atom)\+xml)["']/i,
    rule: 'green_yellow',
  },
  {
    key: 'ads.has_ads_txt',
    family: 'ads',
    describe: '/ads.txt published',
    kind: 'path_status',
    urlPath: '/ads.txt',
    rule: 'green_yellow',
  },
  {
    key: 'contact.has_contact_email',
    family: 'contact',
    describe: 'mailto: link present',
    kind: 'head_regex',
    pattern: /mailto:[\w.\-+]+@/i,
    rule: 'green_yellow',
  },
  {
    key: 'legal.has_privacy_page',
    family: 'legal',
    describe: '/privacy returns 200',
    kind: 'path_status',
    urlPath: '/privacy',
    rule: 'green_yellow',
  },
  {
    key: 'legal.has_terms_page',
    family: 'legal',
    describe: '/terms returns 200',
    kind: 'path_status',
    urlPath: '/terms',
    rule: 'green_yellow',
  },
  {
    key: 'legal.has_security_txt',
    family: 'legal',
    describe: '/.well-known/security.txt returns 200',
    kind: 'path_status',
    urlPath: '/.well-known/security.txt',
    rule: 'green_yellow',
  },
  {
    key: 'legal.has_affiliate_disclosure',
    family: 'legal',
    describe: 'affiliate/Amazon Associates disclosure present',
    kind: 'body_regex',
    pattern: /amazon associate|affiliate (link|disclosure)|earn.{0,40}commission/i,
    rule: 'green_yellow',
  },
  {
    key: 'branding.has_favicon',
    family: 'branding',
    describe: '/favicon.ico returns 200',
    kind: 'path_status',
    urlPath: '/favicon.ico',
    rule: 'green_yellow',
  },
  {
    key: 'branding.has_about_page',
    family: 'branding',
    describe: '/about returns 200',
    kind: 'path_status',
    urlPath: '/about',
    rule: 'green_yellow',
  },
  {
    key: 'http.ga4_present',
    family: 'http',
    describe: 'GA4 tag in <head>',
    kind: 'head_regex',
    pattern: /gtag\(['"]config['"],\s*['"]G-|googletagmanager\.com\/gtag\/js\?id=G-/i,
    rule: 'green_yellow',
  },
  {
    key: 'http.adsense_present',
    family: 'http',
  ...
  {
    key: 'http.meta_pixel_present',
    family: 'http',
    describe: 'Meta Pixel in <head>',
    kind: 'head_regex',
    pattern: /connect\.facebook\.net.*fbevents|fbq\(['"]init['"]/i,
    rule: 'green_yellow',
  },
  {
    key: 'http.gtm_present',
    family: 'http',
...
async function fetchText(url) {
  try {
    const r = await fetch(url, {
      ...
      headers: { 'User-Agent': 'fleet-dashboard/sitefacts' },
    });
...
async function sweep(sites) {
  ...
  async function worker() {
    for (let s = queue.shift(); s; s = queue.shift()) {
      try {
        next[s] = {
          facts: { ...(await collectSite(s)), ...amzFactsFor(s) },
          checkedAt: Date.now(),
        };
      } catch {
        /* leave previous cache entry for this site */
      }
    }
  }
...
function start(getSites) {
  loadCacheFromDisk();
  const tick = () => {
    sweep(getSites()).catch(() => {
      /* swallow; cache simply goes stale */
    });
  };
  tick();
...
function validateManual(key, value) {
  if (!MANUAL_KEY_RE.test(key))
    throw httpErr(400, "key must be letters, numbers, '.', '_', '-' only");
  if (typeof value !== 'string' || !value.trim()) throw httpErr(400, 'value is required');
  if (value.length > MAX_MANUAL_VALUE_LEN)
    throw httpErr(400, `value too long (max ${MAX_MANUAL_VALUE_LEN} chars)`);
  if (/[\x00-\x08\x0a-\x1f\x7f]/.test(value))
    throw httpErr(400, 'value contains control characters');
}
...
function manualFor(site) {
  return loadManual()[site] || {};
}

// The anonymous HTTP scrape cannot see tags injected after consent. When the
// browser compliance scan has confirmed both GA4 and consent gating, its
// result is authoritative for the GA4 presence fact. Keep this override at
// read/scoring time so an old HTTP cache entry is corrected immediately and
// the raw scrape evidence remains available on disk for diagnosis.
...
function effectiveFacts(facts, complianceRow) {
  const effective = { ...facts };
  const checks = complianceRow && complianceRow.checks;
  if (
    checks &&
    checks.ga4 === true &&
    checks.gaConsentGated === true &&
    effective['http.ga4_present'] !== true
  ) {
    effective['http.ga4_present'] = true;
  }
  return effective;
...
function matrix(sites) {
  ...
  const rows = sites.map(site => {
    ...
    const facts = effectiveFacts((cached && cached.facts) || {}, compliance[site]);
    const cells = {};
...
function siteDetail(site) {
  ...
  const amz = {
    asin_count: facts['amz.asin_count'] ?? null,
    oos_count: facts['amz.oos_count'] ?? null,
    delisted_count: facts['amz.delisted_count'] ?? null,
    last_scan: facts['amz.last_scan'] ?? null,
  };
  ...
  return {
    ...
    rows,
    amz,
    tlsExpiryDays: tlsExpiryDaysVal,
    checkedAt: cached ? cached.checkedAt : null,
    manual: manualFor(site),
  };
}
...
module.exports = {
  ...
  setManualFact,
  deleteManualFact,
  _effectiveFacts: effectiveFacts,
};
