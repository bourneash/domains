...
if [ -f "$DB" ]; then
  if command -v sqlite3 >/dev/null 2>&1; then
    sqlite3 "$DB" ".backup '$DEST/images-$STAMP.db'"
  else
    # No sqlite3 binary — fall back to Python's backup API (always present).
    python3 - "$DB" "$DEST/images-$STAMP.db" <<'PY'
import sqlite3, sys
...
if [ -d "$BLOBS" ]; then
  tar -czf "$DEST/blobs-$STAMP.tar.gz" -C "$(dirname "$BLOBS")" "$(basename "$BLOBS")"
