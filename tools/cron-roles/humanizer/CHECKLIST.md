...
Before you finish, re-read your draft against the AI-writing patterns below
(verbatim from the `blader/humanizer` skill, MIT licensed — see
`LICENSE.txt` in this directory — itself based on Wikipedia's "Signs of AI
writing"). This is additive to this role's own voice/anti-slop rules,
never a replacement:

- This role's existing voice and anti-slop rules win over anything below if
  they ever conflict (e.g. a site-specific em-dash ban, a required "cons"
  section, a banned-phrase list).
- Do not invent a fact, name, number, price, date, or source to satisfy a
  pattern below. If a specific detail is missing, write a plainer sentence
  instead of a vaguer one.
- Section 14 (em dashes) duplicates what most site roles already require —
  nothing new there if this role already bans em dashes.
...
---
Source: `blader/humanizer` (MIT, see `LICENSE.txt` in this directory),
