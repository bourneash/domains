...
description: Install (or maintain) the autonomous Engineer cron role on any portfolio site under /home/jesse/projects/domains/sites/. The engineer runs every 30 minutes (:12/:42), true-render health-checks the live site (Playwright in-container), sweeps git/Cloudflare/task-board with zero Claude turns, posts a throttled 👍 Slack heartbeat when healthy+idle, and invokes Sonnet only to fix safe issues behind an authoritative build gate. The Claude work pass is serialized per-site by a time-based lock (ops/.locks/engineer-work.lock.d, auto-reclaims) so two engineers never run on the same site at once. Use when the user asks to "add/install the engineer role", "give <site> a health-check agent", "wire the engineer", or "update the engineer". Stamps from the americastrikes reference, wires cron + run-role + worker Dockerfile, makes siblings aware, and rebuilds+verifies the cron line is live (the sinderella guard).
---
